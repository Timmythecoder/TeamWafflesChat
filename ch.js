...
const roomPath = room_name && room_name.startsWith('gc_')
    ? `group_chat_messages/${room_name}`
    : `room-messages/${room_name}`;
...
function markUnread() {
    ...
    if (room_name && room_name.startsWith('gc_')) {
        // Group chats: mark unread in group_chats_index for all members except sender
        // We don't know all member UIDs here without a read, so dashboard listener handles it
        return;
    }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      // Mark chat as read if it's a DM
  ...
  async function checkRoomAccess() {
    ...
    try {
      // Group chats live in group_chats/{gcId}/meta, not rooms/
      if (room_name.startsWith('gc_')) {
        const gcMetaRef = ref(database, `group_chats/${room_name}/meta`);
        const s = await get(gcMetaRef);
        if (!s.exists()) {
        ...
        if (!meta.members || !meta.members[currentUserUID]) {
          alert("You are not a member of this group chat.");
          location.href = "dashboard.html";
          return false;
        }
